...
@interface MercuryPlayerPresentView : MercuryPlayerView
...
@implementation MercuryPlayerPresentView
...
@interface MercuryAVPlayerManager () {
    id _timeObserver;
    id _itemEndObserver;
    MercuryKVOController *_playerItemKVO;
}
...
@implementation MercuryAVPlayerManager
...
- (instancetype)init {
    self = [super init];
    if (self) {
        _scalingMode = MercuryPlayerScalingModeAspectFit;
        _shouldAutoPlay = YES;
    }
    return self;
}
...
- (void)prepareToPlay {
    ...
    self.loadState = MercuryPlayerLoadStatePrepare;
...
- (void)play {
    if (!_isPreparedToPlay) {
        ...
        self->_isPlaying = YES;
        self.playState = MercuryPlayerPlayStatePlaying;
    }
}
...
- (void)pause {
    [self.player pause];
    self->_isPlaying = NO;
    self.playState = MercuryPlayerPlayStatePaused;
...
- (void)stop {
    ...
    self.loadState = MercuryPlayerLoadStateUnknown;
    self.playState = MercuryPlayerPlayStatePlayStopped;
...
- (void)initializePlayer {
    ...
    MercuryPlayerPresentView *presentView = (MercuryPlayerPresentView *)self.view;
...
- (void)bufferingSomeSecond {
    ...
    if (self.isBuffering || self.playState == MercuryPlayerPlayStatePlayStopped) return;
    /// 没有网络
    if ([MercuryReachabilityManager sharedManager].networkReachabilityStatus == MercuryReachabilityStatusNotReachable) return;
    ...
    dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(3.0 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
        ...
        if (!self.isPlaying && self.loadState == MercuryPlayerLoadStateStalled) {
            self.isBuffering = NO;
            return;
        }
        [self play];
...
- (void)itemObserving {
    [_playerItemKVO safelyRemoveAllObservers];
    _playerItemKVO = [[MercuryKVOController alloc] initWithTarget:_playerItem];
    ...
    _timeObserver = [self.player addPeriodicTimeObserverForInterval:interval queue:dispatch_get_main_queue() usingBlock:^(CMTime time) {
        ...
        if (self.isPlaying && self.loadState == MercuryPlayerLoadStateStalled) self.player.rate = self.rate;
        ...
        if ([keyPath isEqualToString:kStatus]) {
            if (self.player.currentItem.status == AVPlayerItemStatusReadyToPlay) {
                if (!self.isReadyToPlay) {
                    ...
                    self.loadState = MercuryPlayerLoadStatePlaythroughOK;
                ...
                NSArray *loadedRanges = self.playerItem.seekableTimeRanges;
                if (loadedRanges.count > 0) {
                    /// Fix https://github.com/renzifeng/issues/475
                    if (self.playerPlayTimeChanged) self.playerPlayTimeChanged(self, self.currentTime, self.totalTime);
                }
            } else if (self.player.currentItem.status == AVPlayerItemStatusFailed) {
                self.playState = MercuryPlayerPlayStatePlayFailed;
            ...
            if (self.playerItem.playbackBufferEmpty) {
                self.loadState = MercuryPlayerLoadStateStalled;
                [self bufferingSomeSecond];
            }
        } else if ([keyPath isEqualToString:kPlaybackLikelyToKeepUp]) {
            // When the buffer is good
            if (self.playerItem.playbackLikelyToKeepUp) {
                self.loadState = MercuryPlayerLoadStatePlayable;
...
- (UIView *)view {
    if (!_view) {
        _view = [[MercuryPlayerPresentView alloc] init];
    }
    return _view;
}
...
- (void)setPlayState:(MercuryPlayerPlaybackState)playState {
    _playState = playState;
    if (self.playerPlayStateChanged) self.playerPlayStateChanged(self, playState);
}

- (void)setLoadState:(MercuryPlayerLoadState)loadState {
...
- (void)setScalingMode:(MercuryPlayerScalingMode)scalingMode {
    _scalingMode = scalingMode;
    MercuryPlayerPresentView *presentView = (MercuryPlayerPresentView *)self.view;
    switch (scalingMode) {
        case MercuryPlayerScalingModeNone:
            presentView.videoGravity = AVLayerVideoGravityResizeAspect;
            break;
        case MercuryPlayerScalingModeAspectFit:
            presentView.videoGravity = AVLayerVideoGravityResizeAspect;
            break;
        case MercuryPlayerScalingModeAspectFill:
            presentView.videoGravity = AVLayerVideoGravityResizeAspectFill;
            break;
        case MercuryPlayerScalingModeFill:
