...
#import "BY_HTTPServer.h"
#import "BY_HTTPConnection.h"
#import "BY_HTTPMessage.h"
#import "BY_HTTPResponse.h"
#import "BY_HTTPAuthenticationRequest.h"
#import "BY_DDNumber.h"
#import "BY_DDRange.h"
#import "BY_DDData.h"
#import "BY_HTTPFileResponse.h"
#import "BY_HTTPAsyncFileResponse.h"
#import "BY_WebSocket.h"
...
@interface BY_HTTPConnection (PrivateAPI)
...
@implementation BY_HTTPConnection
...
- (id)initWithAsyncSocket:(BY_GCDAsyncSocket *)newSocket configuration:(BY_HTTPConfig *)aConfig {
	...
	if ((self = [super init])) {
		...
		request = [[BY_HTTPMessage alloc] initEmptyRequest];
...
- (BOOL)supportsMethod:(NSString *)method atPath:(NSString *)path
{
	...
	// 
	// For more information, you can always access the BY_HTTPMessage request variable.
	// 
...
- (BOOL)isAuthenticated
{
	...
	BY_HTTPAuthenticationRequest *auth = [[BY_HTTPAuthenticationRequest alloc] initWithRequest:request];
...
**/
- (void)addDigestAuthChallenge:(BY_HTTPMessage *)response
{
	HTTPLogTrace();
...
**/
- (void)addBasicAuthChallenge:(BY_HTTPMessage *)response
{
	HTTPLogTrace();
...
/**
 * This method is called by the BY_HTTPServer if it is asked to stop.
...
- (void)startReadingRequest
{
	...
	[asyncSocket readDataToData:[BY_GCDAsyncSocket CRLFData]
...
- (BOOL)parseRangeRequest:(NSString *)rangeHeader withContentLength:(UInt64)contentLength
{
	...
	// Note: We store all range values in the form of BY_DDRange structs, wrapped in NSValue objects.
	// Since BY_DDRange consists of UInt64 values, the range extends up to 16 exabytes.
	...
	for (i = 0; i < [rangeComponents count]; i++)
	{
		...
		if (dashRange.location == NSNotFound)
		{
			...
			[ranges addObject:[NSValue valueWithBY_DDRange:BY_DDMakeRange(byteIndex, 1)]];
		}
		else
		{
			...
			if (!hasR1)
			{
				...
				[ranges addObject:[NSValue valueWithBY_DDRange:BY_DDMakeRange(startIndex, r2)]];
			}
			else if (!hasR2)
			{
				// We're dealing with a "[#]-" range
				...
				[ranges addObject:[NSValue valueWithBY_DDRange:BY_DDMakeRange(r1, contentLength - r1)]];
			}
			else
			{
				...
				[ranges addObject:[NSValue valueWithBY_DDRange:BY_DDMakeRange(r1, r2 - r1 + 1)]];
			}
		}
	}
	...
	{
		BY_DDRange range1 = [[ranges objectAtIndex:i] ddrangeValue];
		
		NSUInteger j;
		for (j = i+1; j < [ranges count]; j++)
		{
			BY_DDRange range2 = [[ranges objectAtIndex:j] ddrangeValue];
			
			BY_DDRange iRange = BY_DDIntersectionRange(range1, range2);
 ...
 * The current request is in the BY_HTTPMessage request variable.
**/
- (void)replyToHTTPRequest
{
	...
	// Check for BY_WebSocket request
	if ([BY_WebSocket isBY_WebSocketRequest:request])
	{
		HTTPLogVerbose(@"isBY_WebSocket");
		
		BY_WebSocket *ws = [self webSocketForURI:uri];
		...
		else
		{
			...
			[[config server] addBY_WebSocket:ws];
			
			// The BY_WebSocket should now be the delegate of the underlying socket.
			// But gracefully handle the situation if it forgot.
			if ([asyncSocket delegate] == self)
			{
				HTTPLogWarn(@"%@[%p]: BY_WebSocket forgot to set itself as socket delegate", THIS_FILE, self);
			...
			{
				// The BY_WebSocket is using the socket,
 ...
 * 
 * Note: The returned BY_HTTPMessage is owned by the sender, who is responsible for releasing it.
**/
- (BY_HTTPMessage *)newUniRangeResponse:(UInt64)contentLength
{
	HTTPLogTrace();
	
	// Status Code 206 - Partial Content
	BY_HTTPMessage *response = [[BY_HTTPMessage alloc] initResponseWithStatusCode:206 description:nil version:HTTPVersion1_1];
	
	BY_DDRange range = [[ranges objectAtIndex:0] ddrangeValue];
	
	NSString *contentLengthStr = [NSString stringWithFormat:@"%qu", range.length];
	[response setHeaderField:@"Content-Length" value:contentLengthStr];
	
	NSString *rangeStr = [NSString stringWithFormat:@"%qu-%qu", range.location, BY_DDMaxRange(range) - 1];
 ...
 * Note: The returned BY_HTTPMessage is owned by the sender, who is responsible for releasing it.
**/
- (BY_HTTPMessage *)newMultiRangeResponse:(UInt64)contentLength
{
	HTTPLogTrace();
	...
	{
		BY_DDRange range = [[ranges objectAtIndex:i] ddrangeValue];
...
- (void)sendResponseHeadersAndBody
{
	...
	// If a response is "chunked", this simply means the BY_HTTPResponse object
	...
	if (!isRangeRequest)
	{
		...
		response = [[BY_HTTPMessage alloc] initResponseWithStatusCode:status description:nil version:HTTPVersion1_1];
		...
		if (!isRangeRequest)
		{
			...
			if ([data length] > 0)
			{
				...
				if (isChunked)
				{
					...
					{
						NSData *footer = [BY_GCDAsyncSocket CRLFData];
 ...
 * working with asynchronous BY_HTTPResponse objects.
**/
...
- (void)continueSendingStandardResponseBody
{
	...
	// or when an asynchronous BY_HTTPResponse object informs us that it has more available data for us to send.
	// In the case of the asynchronous BY_HTTPResponse, we don't want to blindly grab the new data,
	...
	// 
	// Note that this does not affect the rate at which the BY_HTTPResponse object may generate data.
	// The BY_HTTPResponse is free to do as it pleases, and this is up to the application's developer.
	// If the memory footprint is a concern, the developer creating the custom BY_HTTPResponse object may freely
	// use the calls to readDataOfLength as an indication to start generating more data.
	// This provides an easy way for the BY_HTTPResponse object to throttle its data allocation in step with the rate
...
- (void)continueSendingMultiRangeResponseBody
{
	...
	// This method is called when either asyncSocket has finished writing one of the response data chunks,
	// or when an asynchronous BY_HTTPResponse object informs us that is has more available data for us to send.
	// In the case of the asynchronous BY_HTTPResponse, we don't want to blindly grab the new data,
	// and shove it onto asyncSocket's write queue.
	// Doing so could negatively affect the memory footprint of the application.
	// Instead, we always ensure that we place no more than READ_CHUNKSIZE bytes onto the write queue.
	...
	BY_DDRange range = [[ranges objectAtIndex:rangeIndex] ddrangeValue];
 ...
 * You may return any object that adopts the BY_HTTPResponse protocol.
 * The BY_HTTPServer comes with two such classes: BY_HTTPFileResponse and BY_HTTPDataResponse.
 * BY_HTTPFileResponse is a wrapper for an NSFileHandle object, and is the preferred way to send a file response.
 * BY_HTTPDataResponse is a wrapper for an NSData object, and may be used to send a custom response.
**/
- (NSObject<BY_HTTPResponse> *)httpResponseForMethod:(NSString *)method URI:(NSString *)path
{
	...
	{
		return [[BY_HTTPFileResponse alloc] initWithFilePath:filePath forConnection:self];
	
		// Use me instead for asynchronous file IO.
		// Generally better for larger files.
		
	//	return [[[BY_HTTPAsyncFileResponse alloc] initWithFilePath:filePath forConnection:self] autorelease];
	}
...
- (BY_WebSocket *)webSocketForURI:(NSString *)path
{
	...
	// Override me to provide custom BY_WebSocket responses.
	// To do so, simply override the base BY_WebSocket implementation, and add your custom functionality.
	// Then return an instance of your custom BY_WebSocket here.
	// 
	// For example:
	// 
	// if ([path isEqualToString:@"/myAwesomeBY_WebSocketStream"])
	// {
	//     return [[[MyBY_WebSocket alloc] initWithRequest:request socket:asyncSocket] autorelease];
	// }
	// 
...
- (void)handleVersionNotSupported:(NSString *)version
{
	...
	BY_HTTPMessage *response = [[BY_HTTPMessage alloc] initResponseWithStatusCode:505 description:nil version:HTTPVersion1_1];
...
- (void)handleAuthenticationFailed
{
	...
	BY_HTTPMessage *response = [[BY_HTTPMessage alloc] initResponseWithStatusCode:401 description:nil version:HTTPVersion1_1];
	[response setHeaderField:@"Content-Length" value:@"0"];
...
- (void)handleInvalidRequest:(NSData *)data
{
	...
	BY_HTTPMessage *response = [[BY_HTTPMessage alloc] initResponseWithStatusCode:400 description:nil version:HTTPVersion1_1];
	[response setHeaderField:@"Content-Length" value:@"0"];
...
- (void)handleUnknownMethod:(NSString *)method
{
	...
	BY_HTTPMessage *response = [[BY_HTTPMessage alloc] initResponseWithStatusCode:405 description:nil version:HTTPVersion1_1];
	[response setHeaderField:@"Content-Length" value:@"0"];
	[response setHeaderField:@"Connection" value:@"close"];
...
- (void)handleResourceNotFound
{
	...
	BY_HTTPMessage *response = [[BY_HTTPMessage alloc] initResponseWithStatusCode:404 description:nil version:HTTPVersion1_1];
	[response setHeaderField:@"Content-Length" value:@"0"];
...
- (void)socket:(BY_GCDAsyncSocket *)sock didReadData:(NSData*)data withTag:(long)tag
{
	...
	else
	{
		...
		else if (tag == HTTP_REQUEST_CHUNK_TRAILER)
		{
			...
			if (![data isEqualToData:[BY_GCDAsyncSocket CRLFData]])
			{
	...
	if (doneSendingResponse)
	{
		...
		else
		{
			if ([self shouldDie])
			{
				...
				// or if the socket gets taken over by someone else like a BY_WebSocket.
...
**/
- (void)socketDidDisconnect:(BY_GCDAsyncSocket *)sock withError:(NSError *)err
{
	HTTPLogTrace();
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
#pragma mark BY_HTTPResponse Notifications
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////

/**
 * This method may be called by asynchronous BY_HTTPResponse objects.
 * That is, BY_HTTPResponse objects that return YES in their "- (BOOL)isAsynchronous" method.
 * 
...
- (void)responseHasAvailableData:(NSObject<BY_HTTPResponse> *)sender
{
	...
	// 
	// We do this to give the BY_HTTPResponse classes the flexibility to call
...
@implementation BY_HTTPConfig

@synthesize server;
@synthesize documentRoot;
@synthesize queue;

- (id)initWithServer:(BY_HTTPServer *)aServer documentRoot:(NSString *)aDocumentRoot
{
	if ((self = [super init]))
	{
		server = aServer;
...
- (id)initWithServer:(BY_HTTPServer *)aServer documentRoot:(NSString *)aDocumentRoot queue:(dispatch_queue_t)q
{
	if ((self = [super init]))
	{
		server = aServer;
