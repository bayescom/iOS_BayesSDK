...
@interface BY_HTTPServer (PrivateAPI)
...
- (id)init
{
	if ((self = [super init]))
	{
		...
		serverQueue = dispatch_queue_create("BY_HTTPServer", NULL);
		...
		asyncSocket = [[BY_GCDAsyncSocket alloc] initWithDelegate:self delegateQueue:serverQueue];
		
		// Use default connection class of HTTPConnection
		connectionClass = [BY_HTTPConnection self];
		...
		[[NSNotificationCenter defaultCenter] addObserver:self
		                                         selector:@selector(webSocketDidDie:)
		                                             name:BY_WebSocketDidDieNotification
		                                           object:nil];
...
- (void)stop:(BOOL)keepExistingConnections
{
	...
	dispatch_sync(serverQueue, ^{ @autoreleasepool {
		...
		if (!keepExistingConnections)
		{
			...
			for (BY_HTTPConnection *connection in connections)
			{
				[connection stop];
			}
			...
			// Stop all BY_WebSocket connections the server owns
			[webSocketsLock lock];
			for (BY_WebSocket *webSocket in webSockets)
			{
				[webSocket stop];
			}
...
- (void)addBY_WebSocket:(BY_WebSocket *)ws
{
	[webSocketsLock lock];
...
**/
- (NSUInteger)numberOfBY_WebSocketConnections
{
	NSUInteger result = 0;
...
- (BY_HTTPConfig *)config
{
	...
	// 
	// Generally this involves overriding the BY_HTTPConfig class to include any custom settings,
	// and then having this method return an instance of 'MyBY_HTTPConfig'.
	...
	return [[BY_HTTPConfig alloc] initWithServer:self documentRoot:documentRoot queue:connectionQueue];
}

- (void)socket:(BY_GCDAsyncSocket *)sock didAcceptNewSocket:(BY_GCDAsyncSocket *)newSocket
{
	BY_HTTPConnection *newConnection = (BY_HTTPConnection *)[[connectionClass alloc] initWithAsyncSocket:newSocket
...
/**
 * This method is automatically called when a notification of type BY_WebSocketDidDieNotification is posted.
