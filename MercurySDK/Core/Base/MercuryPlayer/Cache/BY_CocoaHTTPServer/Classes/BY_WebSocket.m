...
@interface BY_WebSocket (PrivateAPI)
...
+ (BOOL)isBY_WebSocketRequest:(BY_HTTPMessage *)request
{
	...
	// 
	// GET /demo HTTP/1.1
	// Upgrade: BY_WebSocket
	// Connection: Upgrade
	// Host: example.com
	// Origin: http://example.com
	// BY_WebSocket-Protocol: sample
	// 
	// 
	// Request (Draft 76):
	//
	...
	// Host: example.com
	// Origin: http://example.com
	// Sec-BY_WebSocket-Protocol: sample
	// Sec-BY_WebSocket-Key1: 4 @1  46546xW%0l 1 5
	// Sec-BY_WebSocket-Key2: 12998 5 Y3 1  .P00
	// 
	// ^n:ds[4U
	...
	BOOL isBY_WebSocket = YES;
	
	if (!upgradeHeaderValue || !connectionHeaderValue) {
		isBY_WebSocket = NO;
	}
	else if (![upgradeHeaderValue caseInsensitiveCompare:@"BY_WebSocket"] == NSOrderedSame) {
		isBY_WebSocket = NO;
	}
	...
	HTTPLogTrace2(@"%@: %@ - %@", THIS_FILE, THIS_METHOD, (isBY_WebSocket ? @"YES" : @"NO"));
	
	return isBY_WebSocket;
}

+ (BOOL)isVersion76Request:(BY_HTTPMessage *)request
{
	NSString *key1 = [request headerField:@"Sec-BY_WebSocket-Key1"];
	NSString *key2 = [request headerField:@"Sec-BY_WebSocket-Key2"];
...
+ (BOOL)isRFC6455Request:(BY_HTTPMessage *)request
{
	NSString *key = [request headerField:@"Sec-BY_WebSocket-Key"];
...
- (id)initWithRequest:(BY_HTTPMessage *)aRequest socket:(BY_GCDAsyncSocket *)socket
{
	HTTPLogTrace();
	...
	if ((self = [super init]))
	{
		...
		websocketQueue = dispatch_queue_create("BY_WebSocket", NULL);
		request = aRequest;
...
/**
 * Starting point for the BY_WebSocket after it has been fully initialized (including subclasses).
 ...
 * The server, in turn, invokes stop on each BY_WebSocket instance.
**/
- (void)stop
{
	// This method is not exactly designed to be overriden.
...
- (void)readRequestBody
{
	...
	NSAssert(isVersion76, @"BY_WebSocket version 75 doesn't contain a request body");
...
- (NSString *)secBY_WebSocketKeyResponseHeaderValue {
	NSString *key = [request headerField: @"Sec-BY_WebSocket-Key"];
...
- (void)sendResponseHeaders
{
	...
	// Upgrade: BY_WebSocket
	// Connection: Upgrade
	// BY_WebSocket-Origin: http://example.com
	// BY_WebSocket-Location: ws://example.com/demo
	// BY_WebSocket-Protocol: sample
	// 
	// 
	// Response (Draft 76):
	//
	// HTTP/1.1 101 BY_WebSocket Protocol Handshake
	// Upgrade: BY_WebSocket
	// Connection: Upgrade
	// Sec-BY_WebSocket-Origin: http://example.com
	// Sec-BY_WebSocket-Location: ws://example.com/demo
	// Sec-BY_WebSocket-Protocol: sample
	// 
	// 8jKS'y:G*Co,Wxa-

	
	BY_HTTPMessage *wsResponse = [[BY_HTTPMessage alloc] initResponseWithStatusCode:101
	                                                              description:@"Web Socket Protocol Handshake"
	                                                                  version:HTTPVersion1_1];
	
	[wsResponse setHeaderField:@"Upgrade" value:@"BY_WebSocket"];
	[wsResponse setHeaderField:@"Connection" value:@"Upgrade"];
	
	// Note: It appears that BY_WebSocket-Origin and BY_WebSocket-Location
	// are required for Google's Chrome implementation to work properly.
	// 
	// If we don't send either header, Chrome will never report the BY_WebSocket as open.
	// If we only send one of the two, Chrome will immediately close the BY_WebSocket.
	// 
	// In addition to this it appears that Chrome's implementation is very picky of the values of the headers.
	// They have to match exactly with what Chrome sent us or it will close the BY_WebSocket.
	
	NSString *originValue = [self originResponseHeaderValue];
	NSString *locationValue = [self locationResponseHeaderValue];
	
	NSString *originField = isVersion76 ? @"Sec-BY_WebSocket-Origin" : @"BY_WebSocket-Origin";
	NSString *locationField = isVersion76 ? @"Sec-BY_WebSocket-Location" : @"BY_WebSocket-Location";
	
	[wsResponse setHeaderField:originField value:originValue];
	[wsResponse setHeaderField:locationField value:locationValue];
	
	NSString *acceptValue = [self secBY_WebSocketKeyResponseHeaderValue];
	if (acceptValue) {
		[wsResponse setHeaderField: @"Sec-BY_WebSocket-Accept" value: acceptValue];
	}
...
- (void)sendResponseBody:(NSData *)d3
{
	...
	NSAssert(isVersion76, @"BY_WebSocket version 75 doesn't contain a response body");
...
- (void)didOpen
{
	...
	// Override me to perform any custom actions once the BY_WebSocket has been opened.
...
- (void)didClose
{
	...
	// Notify BY_HTTPServer
	[[NSNotificationCenter defaultCenter] postNotificationName:BY_WebSocketDidDieNotification object:self];
}

#pragma mark BY_WebSocket Frame

- (BOOL)isValidBY_WebSocketFrame:(UInt8)frame
{
...
- (void)socket:(BY_GCDAsyncSocket *)sock didReadData:(NSData *)data withTag:(long)tag
{
	...
	else if (tag == TAG_PAYLOAD_PREFIX)
	{
		...
		if ([self isValidBY_WebSocketFrame: frame])
		{
...
- (void)socketDidDisconnect:(BY_GCDAsyncSocket *)sock withError:(NSError *)error
{
