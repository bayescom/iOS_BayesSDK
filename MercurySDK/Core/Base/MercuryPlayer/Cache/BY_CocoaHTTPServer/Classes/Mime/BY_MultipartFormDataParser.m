...
#import "BY_MultipartFormDataParser.h"
#import "BY_DDData.h"
...
//-----------------------------------------------------------------
// interface BY_MultipartFormDataParser (private)
//-----------------------------------------------------------------
...
//-----------------------------------------------------------------
// implementation BY_MultipartFormDataParser
//-----------------------------------------------------------------
...
- (id) initWithBoundary:(NSString*) boundary formEncoding:(NSStringEncoding) _formEncoding {
	...
	if( nil == boundary ) {
		HTTPLogWarn(@"BY_MultipartFormDataParser: init with zero boundary");
		return nil;
	}
...
- (BOOL) appendData:(NSData *)data { 
    ...
    if( nil == boundaryData ) {
		HTTPLogError(@"BY_MultipartFormDataParser: Trying to parse multipart without specifying a valid boundary");
		assert(false);
        return NO;
    }
		...
		if( checkForContentEnd ) {
			if(	pendingData.length >= 2 ) {
				if( *(uint16_t*)(pendingData.bytes + offset) == 0x2D2D ) {
					...
					HTTPLogVerbose(@"BY_MultipartFormDataParser: End of multipart message");
					waitingForCRLF = YES;
					reachedEpilogue = YES;
					offset+= 2;
				}
	...
	while( true ) {
		...
		if( nil == currentHeader ) {
			...
			if( -1 == headerEnd ) {
				...
				currentHeader = [[BY_MultipartMessageHeader alloc] initWithData:headerData formEncoding:formEncoding];

				if( nil == currentHeader ) {
					// we've found the data is in wrong format.
					HTTPLogError(@"BY_MultipartFormDataParser: BY_MultipartFormDataParser: wrong input format, coulnd't get a valid header");
					return NO;
				}
				...
				HTTPLogVerbose(@"BY_MultipartFormDataParser: BY_MultipartFormDataParser: Retrieved part header.");
			}
		...
		if( contentEnd == -1 ) {
			...
			NSData* decodedData = [BY_MultipartFormDataParser decodedDataFromData:[NSData dataWithBytesNoCopy:(char*)workingData.bytes + offset length:workingData.length - offset - sizeToLeavePending freeWhenDone:NO] encoding:currentEncoding];
			
			if( [delegate respondsToSelector:@selector(processContent:WithHeader:)] ) {
				HTTPLogVerbose(@"BY_MultipartFormDataParser: Processed %"FMTNSINT" bytes of body",sizeToPass);
			...
			if( [delegate respondsToSelector:@selector(processEndOfPartWithHeader:)] ){
				...
				HTTPLogVerbose(@"BY_MultipartFormDataParser: End of body part");
			}
			currentHeader = nil;
...
- (int) offsetTillNewlineSinceOffset:(int) offset inData:(NSData*) data {
	...
	while ( *(uint16_t*)(bytes + offset) != 0x0A0D ) {
		...
		if( !isspace(*(bytes+offset)) ) {
			HTTPLogWarn(@"BY_MultipartFormDataParser: Warning, non-whitespace character '%c' between boundary bytes and CRLF in boundary line",*(bytes+offset) );
		}
		if( !isspace(*(bytes+offset+1)) ) {
			HTTPLogWarn(@"BY_MultipartFormDataParser: Warning, non-whitespace character '%c' between boundary bytes and CRLF in boundary line",*(bytes+offset+1) );
		}
#endif
		offset++;
		if( offset >= length ) {
	...
	if( offset == dataLength ) {
		...
		if( sizeToProcess > 0) {
			if( [delegate respondsToSelector:@selector(processPreambleData:)] ) {
				...
				HTTPLogVerbose(@"BY_MultipartFormDataParser: processed preamble");
			}
...
//	http://tools.ietf.org/html/rfc2045#section-6.7
	...
	while( count < length ) {
		if( bytes[count] == '=' ) {
			...
			if( length < 3 ) {
				HTTPLogWarn(@"BY_MultipartFormDataParser: warning, trailing '=' in quoted printable data");
			}
			// soft newline
			if( bytes[0] == '\r' ) {
				bytes += 1;
		...
		if( (unsigned char) bytes[count] > 126 ) {
			HTTPLogWarn(@"BY_MultipartFormDataParser: Warning, character with code above 126 appears in quoted printable encoded data");
		}
#endif
