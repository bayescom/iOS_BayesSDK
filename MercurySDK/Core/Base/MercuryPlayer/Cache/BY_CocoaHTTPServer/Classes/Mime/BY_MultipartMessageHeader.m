...
//-----------------------------------------------------------------
// implementation BY_MultipartMessageHeader
//-----------------------------------------------------------------
...
- (id) initWithData:(NSData *)data formEncoding:(NSStringEncoding) formEncoding {
	...
	while( offset < length - 2 ) {
		...
		if( (*(uint16_t*) (bytes+offset)  == fields_separator) && ((offset == length - 2) || !(isspace(bytes[offset+2])) )) {
			...
			BY_MultipartMessageHeaderField* field = [[BY_MultipartMessageHeaderField alloc] initWithData: fieldData  contentEncoding:formEncoding];
			if( field ) {
				[fields setObject:field forKey:field.name];
				HTTPLogVerbose(@"BY_MultipartFormDataParser: Processed Header field '%@'",field.name);
			}
			else {
				NSString* fieldStr = [[NSString  alloc] initWithData:fieldData encoding:NSASCIIStringEncoding];
				HTTPLogWarn(@"BY_MultipartFormDataParser: Failed to parse MIME header field. Input ASCII string:%@",fieldStr);
			}
