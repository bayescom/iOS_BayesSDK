...
#ifndef BY_GCDAsyncSocketLoggingEnabled
#define BY_GCDAsyncSocketLoggingEnabled 0
#endif

#if BY_GCDAsyncSocketLoggingEnabled
...
#define LogAsync   YES
#define LogContext BY_GCDAsyncSocketLoggingContext
...
#ifndef BY_GCDAsyncSocketLogLevel
#define BY_GCDAsyncSocketLogLevel LOG_LEVEL_VERBOSE
#endif

// Log levels : off, error, warn, info, verbose
static const int logLevel = BY_GCDAsyncSocketLogLevel;
...
NSString *const BY_GCDAsyncSocketException = @"BY_GCDAsyncSocketException";
NSString *const BY_GCDAsyncSocketErrorDomain = @"BY_GCDAsyncSocketErrorDomain";

NSString *const BY_GCDAsyncSocketQueueName = @"BY_GCDAsyncSocket";
NSString *const BY_GCDAsyncSocketThreadName = @"BY_GCDAsyncSocket-CFStream";

NSString *const BY_GCDAsyncSocketManuallyEvaluateTrust = @"BY_GCDAsyncSocketManuallyEvaluateTrust";
#if TARGET_OS_IPHONE
NSString *const BY_GCDAsyncSocketUseCFStreamForTLS = @"BY_GCDAsyncSocketUseCFStreamForTLS";
#endif
NSString *const BY_GCDAsyncSocketSSLPeerID = @"BY_GCDAsyncSocketSSLPeerID";
NSString *const BY_GCDAsyncSocketSSLProtocolVersionMin = @"BY_GCDAsyncSocketSSLProtocolVersionMin";
NSString *const BY_GCDAsyncSocketSSLProtocolVersionMax = @"BY_GCDAsyncSocketSSLProtocolVersionMax";
NSString *const BY_GCDAsyncSocketSSLSessionOptionFalseStart = @"BY_GCDAsyncSocketSSLSessionOptionFalseStart";
NSString *const BY_GCDAsyncSocketSSLSessionOptionSendOneByteRecord = @"BY_GCDAsyncSocketSSLSessionOptionSendOneByteRecord";
NSString *const BY_GCDAsyncSocketSSLCipherSuites = @"BY_GCDAsyncSocketSSLCipherSuites";
#if !TARGET_OS_IPHONE
NSString *const BY_GCDAsyncSocketSSLDiffieHellmanParameters = @"BY_GCDAsyncSocketSSLDiffieHellmanParameters";
#endif

enum BY_GCDAsyncSocketFlags
{
...
@interface BY_GCDAsyncSocketPreBuffer : NSObject
{
	uint8_t *preBuffer;
...
@implementation BY_GCDAsyncSocketPreBuffer
...
/**
 * The BY_GCDAsyncReadPacket encompasses the instructions for any given read.
 * The content of a read packet allows the code to determine if we're:
 *  - reading to a certain length
 *  - reading to a certain separator
 *  - or simply reading the first chunk of available data
**/
@interface BY_GCDAsyncReadPacket : NSObject
{
  @public
...
- (NSUInteger)readLengthForTermWithPreBuffer:(BY_GCDAsyncSocketPreBuffer *)preBuffer found:(BOOL *)foundPtr;

- (NSInteger)searchForTermAfterPreBuffering:(ssize_t)numBytes;

@end

@implementation BY_GCDAsyncReadPacket
...
/**
 * The BY_GCDAsyncWritePacket encompasses the instructions for any given write.
**/
@interface BY_GCDAsyncWritePacket : NSObject
{
  @public
	NSData *buffer;
	NSUInteger bytesDone;
...
@implementation BY_GCDAsyncWritePacket
...
/**
 * The BY_GCDAsyncSpecialPacket encompasses special instructions for interruptions in the read/write queues.
 * This class my be altered to support more than just TLS in the future.
**/
@interface BY_GCDAsyncSpecialPacket : NSObject
{
  @public
...
@implementation BY_GCDAsyncSpecialPacket
...
@implementation BY_GCDAsyncSocket
{
	uint32_t flags;
	uint16_t config;
	
	__weak id<BY_GCDAsyncSocketDelegate> delegate;
	...
	BY_GCDAsyncReadPacket *currentRead;
	BY_GCDAsyncWritePacket *currentWrite;
	
	unsigned long socketFDBytesAvailable;
	
	BY_GCDAsyncSocketPreBuffer *preBuffer;
	...
	BY_GCDAsyncSocketPreBuffer *sslPreBuffer;
...
- (id)initWithDelegate:(id<BY_GCDAsyncSocketDelegate>)aDelegate delegateQueue:(dispatch_queue_t)dq socketQueue:(dispatch_queue_t)sq
{
	if((self = [super init]))
	{
		delegate = aDelegate;
		...
		{
			socketQueue = dispatch_queue_create([BY_GCDAsyncSocketQueueName UTF8String], NULL);
		}
		...
		preBuffer = [[BY_GCDAsyncSocketPreBuffer alloc] initWithCapacity:(1024 * 4)];
...
+ (nullable instancetype)socketFromConnectedSocketFD:(int)socketFD delegate:(nullable id<BY_GCDAsyncSocketDelegate>)aDelegate delegateQueue:(nullable dispatch_queue_t)dq error:(NSError**)error {
  return [self socketFromConnectedSocketFD:socketFD delegate:aDelegate delegateQueue:dq socketQueue:NULL error:error];
}

+ (nullable instancetype)socketFromConnectedSocketFD:(int)socketFD delegate:(nullable id<BY_GCDAsyncSocketDelegate>)aDelegate delegateQueue:(nullable dispatch_queue_t)dq socketQueue:(nullable dispatch_queue_t)sq error:(NSError* __autoreleasing *)error
{
  __block BOOL errorOccured = NO;
  
  BY_GCDAsyncSocket *socket = [[[self class] alloc] initWithDelegate:aDelegate delegateQueue:dq socketQueue:sq];
  ...
  dispatch_sync(socket->socketQueue, ^{ @autoreleasepool {
    ...
    if (retVal)
    {
      NSString *errMsg = NSLocalizedStringWithDefaultValue(@"BY_GCDAsyncSocketOtherError",
                                                           @"BY_GCDAsyncSocket", [NSBundle mainBundle],
      ...
      errorOccured = YES;
      if (error)
        *error = [NSError errorWithDomain:BY_GCDAsyncSocketErrorDomain code:BY_GCDAsyncSocketOtherError userInfo:userInfo];
      return;
    }
...
- (void)getDelegate:(id<BY_GCDAsyncSocketDelegate> *)delegatePtr delegateQueue:(dispatch_queue_t *)delegateQueuePtr
{
	if (dispatch_get_specific(IsOnSocketQueueOrTargetQueueKey))
	{
...
- (BOOL)acceptOnInterface:(NSString *)inInterface port:(uint16_t)port error:(NSError **)errPtr
{
	...
	dispatch_block_t block = ^{ @autoreleasepool {
		...
		if (enableIPv4)
		{
			...
			__weak BY_GCDAsyncSocket *weakSelf = self;
			
            dispatch_source_set_event_handler(self->accept4Source, ^{ @autoreleasepool {
			#pragma clang diagnostic push
			#pragma clang diagnostic warning "-Wimplicit-retain-self"
				
				__strong BY_GCDAsyncSocket *strongSelf = weakSelf;
...
- (BOOL)doAccept:(int)parentSocketFD
{
	...
	if (delegateQueue)
	{
		...
		dispatch_async(delegateQueue, ^{ @autoreleasepool {
			...
			// Create BY_GCDAsyncSocket instance for accepted socket
			
			BY_GCDAsyncSocket *acceptedSocket = [[[self class] alloc] initWithDelegate:theDelegate
...
- (NSError *)badConfigError:(NSString *)errMsg
{
	...
	return [NSError errorWithDomain:BY_GCDAsyncSocketErrorDomain code:BY_GCDAsyncSocketBadConfigError userInfo:userInfo];
}
...
- (NSError *)badParamError:(NSString *)errMsg
{
	...
	return [NSError errorWithDomain:BY_GCDAsyncSocketErrorDomain code:BY_GCDAsyncSocketBadParamError userInfo:userInfo];
}
...
{
	NSString *errMsg = NSLocalizedStringWithDefaultValue(@"BY_GCDAsyncSocketConnectTimeoutError",
	                                                     @"BY_GCDAsyncSocket", [NSBundle mainBundle],
	                                                     @"Attempt to connect to host timed out", nil);
	
	NSDictionary *userInfo = [NSDictionary dictionaryWithObject:errMsg forKey:NSLocalizedDescriptionKey];
	
	return [NSError errorWithDomain:BY_GCDAsyncSocketErrorDomain code:BY_GCDAsyncSocketConnectTimeoutError userInfo:userInfo];
}
...
{
	NSString *errMsg = NSLocalizedStringWithDefaultValue(@"BY_GCDAsyncSocketReadMaxedOutError",
														 @"BY_GCDAsyncSocket", [NSBundle mainBundle],
														 @"Read operation reached set maximum length", nil);
	
	NSDictionary *info = [NSDictionary dictionaryWithObject:errMsg forKey:NSLocalizedDescriptionKey];
	
	return [NSError errorWithDomain:BY_GCDAsyncSocketErrorDomain code:BY_GCDAsyncSocketReadMaxedOutError userInfo:info];
}
...
{
	NSString *errMsg = NSLocalizedStringWithDefaultValue(@"BY_GCDAsyncSocketReadTimeoutError",
	                                                     @"BY_GCDAsyncSocket", [NSBundle mainBundle],
	                                                     @"Read operation timed out", nil);
	
	NSDictionary *userInfo = [NSDictionary dictionaryWithObject:errMsg forKey:NSLocalizedDescriptionKey];
	
	return [NSError errorWithDomain:BY_GCDAsyncSocketErrorDomain code:BY_GCDAsyncSocketReadTimeoutError userInfo:userInfo];
}
...
{
	NSString *errMsg = NSLocalizedStringWithDefaultValue(@"BY_GCDAsyncSocketWriteTimeoutError",
	                                                     @"BY_GCDAsyncSocket", [NSBundle mainBundle],
	                                                     @"Write operation timed out", nil);
	
	NSDictionary *userInfo = [NSDictionary dictionaryWithObject:errMsg forKey:NSLocalizedDescriptionKey];
	
	return [NSError errorWithDomain:BY_GCDAsyncSocketErrorDomain code:BY_GCDAsyncSocketWriteTimeoutError userInfo:userInfo];
}

- (NSError *)connectionClosedError
{
	NSString *errMsg = NSLocalizedStringWithDefaultValue(@"BY_GCDAsyncSocketClosedError",
	                                                     @"BY_GCDAsyncSocket", [NSBundle mainBundle],
	                                                     @"Socket closed by remote peer", nil);
	
	NSDictionary *userInfo = [NSDictionary dictionaryWithObject:errMsg forKey:NSLocalizedDescriptionKey];
	
	return [NSError errorWithDomain:BY_GCDAsyncSocketErrorDomain code:BY_GCDAsyncSocketClosedError userInfo:userInfo];
}
...
- (NSError *)otherError:(NSString *)errMsg
{
	...
	return [NSError errorWithDomain:BY_GCDAsyncSocketErrorDomain code:BY_GCDAsyncSocketOtherError userInfo:userInfo];
}
...
- (BOOL)usingCFStreamForTLS
{
	...
	{
		// The startTLS method was given the BY_GCDAsyncSocketUseCFStreamForTLS flag.
                        ...
                        tag:(long)tag
{
	...
	BY_GCDAsyncReadPacket *packet = [[BY_GCDAsyncReadPacket alloc] initWithData:buffer
	                                                          startOffset:offset
	                                                            maxLength:length
	                                                              timeout:timeout
	                                                           readLength:0
...
- (float)progressOfReadReturningTag:(long *)tagPtr bytesDone:(NSUInteger *)donePtr total:(NSUInteger *)totalPtr
{
	...
	dispatch_block_t block = ^{
        ...
        if (!self->currentRead || ![self->currentRead isKindOfClass:[BY_GCDAsyncReadPacket class]])
		{
...
- (void)maybeDequeueRead
{
	...
	if ((currentRead == nil) && (flags & kConnected))
	{
		if ([readQueue count] > 0)
		{
			...
			if ([currentRead isKindOfClass:[BY_GCDAsyncSpecialPacket class]])
			{
				LogVerbose(@"Dequeued BY_GCDAsyncSpecialPacket");
			...
			{
				LogVerbose(@"Dequeued BY_GCDAsyncReadPacket");
...
- (void)doReadData
{
	...
	if ([self usingCFStreamForTLS])
	{
		...
		// Requested CFStream, rather than SecureTransport, for TLS (via BY_GCDAsyncSocketUseCFStreamForTLS)
...
- (void)completeCurrentRead
{
	...
	{
		BY_GCDAsyncReadPacket *theRead = currentRead; // Ensure currentRead retained since result may not own buffer
...
- (void)doReadTimeout
{
	...
	{
		BY_GCDAsyncReadPacket *theRead = currentRead;
...
- (void)writeData:(NSData *)data withTimeout:(NSTimeInterval)timeout tag:(long)tag
{
	...
	BY_GCDAsyncWritePacket *packet = [[BY_GCDAsyncWritePacket alloc] initWithData:data timeout:timeout tag:tag];
...
- (float)progressOfWriteReturningTag:(long *)tagPtr bytesDone:(NSUInteger *)donePtr total:(NSUInteger *)totalPtr
{
	...
	dispatch_block_t block = ^{
        ...
        if (!self->currentWrite || ![self->currentWrite isKindOfClass:[BY_GCDAsyncWritePacket class]])
		{
...
- (void)maybeDequeueWrite
{
	...
	if ((currentWrite == nil) && (flags & kConnected))
	{
		if ([writeQueue count] > 0)
		{
			...
			if ([currentWrite isKindOfClass:[BY_GCDAsyncSpecialPacket class]])
			{
				LogVerbose(@"Dequeued BY_GCDAsyncSpecialPacket");
			...
			{
				LogVerbose(@"Dequeued BY_GCDAsyncWritePacket");
...
- (void)doWriteData
{
	...
	// Note: This method is not called if currentWrite is a BY_GCDAsyncSpecialPacket (startTLS packet)
...
- (void)doWriteTimeout
{
	...
	{
		BY_GCDAsyncWritePacket *theWrite = currentWrite;
...
- (void)startTLS:(NSDictionary *)tlsSettings
{
	...
	BY_GCDAsyncSpecialPacket *packet = [[BY_GCDAsyncSpecialPacket alloc] initWithTLSSettings:tlsSettings];
...
- (void)maybeStartTLS
{
	...
	if ((flags & kStartingReadTLS) && (flags & kStartingWriteTLS))
	{
		...
		#if TARGET_OS_IPHONE
		{
			BY_GCDAsyncSpecialPacket *tlsPacket = (BY_GCDAsyncSpecialPacket *)currentRead;
            NSDictionary *tlsSettings = @{};
            if (tlsPacket) {
                tlsSettings = tlsPacket->tlsSettings;
            }
			NSNumber *value = [tlsSettings objectForKey:BY_GCDAsyncSocketUseCFStreamForTLS];
...
{
	BY_GCDAsyncSocket *asyncSocket = (__bridge BY_GCDAsyncSocket *)connection;
...
- (void)ssl_startTLS
{
	...
	BOOL shouldManuallyEvaluateTrust = [[tlsSettings objectForKey:BY_GCDAsyncSocketManuallyEvaluateTrust] boolValue];
	...
	//  3. BY_GCDAsyncSocketSSLPeerID
	//  4. BY_GCDAsyncSocketSSLProtocolVersionMin
	//  5. BY_GCDAsyncSocketSSLProtocolVersionMax
	//  6. BY_GCDAsyncSocketSSLSessionOptionFalseStart
	//  7. BY_GCDAsyncSocketSSLSessionOptionSendOneByteRecord
	//  8. BY_GCDAsyncSocketSSLCipherSuites
	//  9. BY_GCDAsyncSocketSSLDiffieHellmanParameters (Mac)
	//
	...
	value = [tlsSettings objectForKey:BY_GCDAsyncSocketSSLPeerID];
	...
	{
		NSAssert(NO, @"Invalid value for BY_GCDAsyncSocketSSLPeerID. Value must be of type NSData."
		             @" (You can convert strings to data using a method like"
		             @" [string dataUsingEncoding:NSUTF8StringEncoding])");
		
		[self closeWithError:[self otherError:@"Invalid value for BY_GCDAsyncSocketSSLPeerID."]];
		return;
	}
	
	// 4. BY_GCDAsyncSocketSSLProtocolVersionMin
	
	value = [tlsSettings objectForKey:BY_GCDAsyncSocketSSLProtocolVersionMin];
	...
	{
		NSAssert(NO, @"Invalid value for BY_GCDAsyncSocketSSLProtocolVersionMin. Value must be of type NSNumber.");
		
		[self closeWithError:[self otherError:@"Invalid value for BY_GCDAsyncSocketSSLProtocolVersionMin."]];
		return;
	}
	
	// 5. BY_GCDAsyncSocketSSLProtocolVersionMax
	
	value = [tlsSettings objectForKey:BY_GCDAsyncSocketSSLProtocolVersionMax];
	if ([value isKindOfClass:[NSNumber class]])
	{
	...
	{
		NSAssert(NO, @"Invalid value for BY_GCDAsyncSocketSSLProtocolVersionMax. Value must be of type NSNumber.");
		
		[self closeWithError:[self otherError:@"Invalid value for BY_GCDAsyncSocketSSLProtocolVersionMax."]];
		return;
	}
	
	// 6. BY_GCDAsyncSocketSSLSessionOptionFalseStart
	
	value = [tlsSettings objectForKey:BY_GCDAsyncSocketSSLSessionOptionFalseStart];
	if ([value isKindOfClass:[NSNumber class]])
	{
	...
	{
		NSAssert(NO, @"Invalid value for BY_GCDAsyncSocketSSLSessionOptionFalseStart. Value must be of type NSNumber.");
		
		[self closeWithError:[self otherError:@"Invalid value for BY_GCDAsyncSocketSSLSessionOptionFalseStart."]];
		return;
	}
	
	// 7. BY_GCDAsyncSocketSSLSessionOptionSendOneByteRecord
	
	value = [tlsSettings objectForKey:BY_GCDAsyncSocketSSLSessionOptionSendOneByteRecord];
	if ([value isKindOfClass:[NSNumber class]])
	{
	...
	{
		NSAssert(NO, @"Invalid value for BY_GCDAsyncSocketSSLSessionOptionSendOneByteRecord."
		             @" Value must be of type NSNumber.");
		
		[self closeWithError:[self otherError:@"Invalid value for BY_GCDAsyncSocketSSLSessionOptionSendOneByteRecord."]];
		return;
	}
	
	// 8. BY_GCDAsyncSocketSSLCipherSuites
	
	value = [tlsSettings objectForKey:BY_GCDAsyncSocketSSLCipherSuites];
	if ([value isKindOfClass:[NSArray class]])
	{
	...
	{
		NSAssert(NO, @"Invalid value for BY_GCDAsyncSocketSSLCipherSuites. Value must be of type NSArray.");
		
		[self closeWithError:[self otherError:@"Invalid value for BY_GCDAsyncSocketSSLCipherSuites."]];
		return;
	}
	
	// 9. BY_GCDAsyncSocketSSLDiffieHellmanParameters
	
	#if !TARGET_OS_IPHONE
	value = [tlsSettings objectForKey:BY_GCDAsyncSocketSSLDiffieHellmanParameters];
	if ([value isKindOfClass:[NSData class]])
	{
	...
	{
		NSAssert(NO, @"Invalid value for BY_GCDAsyncSocketSSLDiffieHellmanParameters. Value must be of type NSData.");
		
		[self closeWithError:[self otherError:@"Invalid value for BY_GCDAsyncSocketSSLDiffieHellmanParameters."]];
		return;
	}
	#endif
	...
	if (value)
	{
		NSAssert(NO, @"Security option unavailable - kCFStreamSSLLevel"
		             @" - You must use BY_GCDAsyncSocketSSLProtocolVersionMin & BY_GCDAsyncSocketSSLProtocolVersionMax");
	...
	sslPreBuffer = [[BY_GCDAsyncSocketPreBuffer alloc] initWithCapacity:(1024 * 4)];
...
- (void)ssl_continueSSLHandshake
{
	...
	else if (status == errSSLPeerAuthCompleted)
	{
		...
		else
		{
			...
			NSString *msg = @"BY_GCDAsyncSocketManuallyEvaluateTrust specified in tlsSettings,"
...
- (void)cf_startTLS
{
	...
	NSAssert([currentRead isKindOfClass:[BY_GCDAsyncSpecialPacket class]], @"Invalid read packet for startTLS");
	NSAssert([currentWrite isKindOfClass:[BY_GCDAsyncSpecialPacket class]], @"Invalid write packet for startTLS");
...
+ (void)startCFStreamThreadIfNeeded
{
	...
	dispatch_once(&predicate, ^{
		...
		cfstreamThreadSetupQueue = dispatch_queue_create("BY_GCDAsyncSocket-CFStreamThreadSetup", DISPATCH_QUEUE_SERIAL);
	});
...
{
	[[NSThread currentThread] setName:BY_GCDAsyncSocketThreadName];
...
+ (void)scheduleCFStreams:(BY_GCDAsyncSocket *)asyncSocket
{
	LogTrace();
...
+ (void)unscheduleCFStreams:(BY_GCDAsyncSocket *)asyncSocket
{
	LogTrace();
	NSAssert([NSThread currentThread] == cfstreamThread, @"Invoked on wrong thread");
...
{
	BY_GCDAsyncSocket *asyncSocket = (__bridge BY_GCDAsyncSocket *)pInfo;
