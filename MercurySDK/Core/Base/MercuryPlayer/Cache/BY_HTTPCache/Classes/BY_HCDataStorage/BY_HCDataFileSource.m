...
@interface BY_HCDataFileSource () <NSLocking>
...
@implementation BY_HCDataFileSource
...
- (instancetype)initWithPath:(NSString *)path range:(BY_HCRange)range readRange:(BY_HCRange)readRange
{
    if (self = [super init])
    {
        BY_HCLogAlloc(self);
        ...
        BY_HCLogDataFileSource(@"%p, Create file source\npath : %@\nrange : %@\nreadRange : %@", self, path, BY_HCStringFromRange(range), BY_HCStringFromRange(readRange));
    }
    return self;
}
...
- (void)prepare
{
    ...
    BY_HCLogDataFileSource(@"%p, Call prepare", self);
...
- (void)close
{
    ...
    self->_closed = YES;
    BY_HCLogDataFileSource(@"%p, Call close", self);
...
- (NSData *)readDataOfLength:(NSUInteger)length
{
    ...
    NSData *data = nil;
    @try {
        long long readLength = BY_HCRangeGetLength(self.readRange);
        ...
        if (data.length > 0) {
            BY_HCLogDataFileSource(@"%p, Read data : %lld, %lld, %lld", self, (long long)data.length, self.readedLength, readLength);
        }
        if (self.readedLength >= readLength) {
            BY_HCLogDataFileSource(@"%p, Read data did finished", self);
            [self destoryReadingHandle];
            self->_finished = YES;
        }
    } @catch (NSException *exception) {
...
- (void)destoryReadingHandle
{
    if (self.readingHandle) {
        @try {
            ...
            BY_HCLogDataFileSource(@"%p, Close exception\nname : %@\nreason : %@\nuserInfo : %@", self, exception.name, exception.reason, exception.userInfo);
        }
