...
@interface BY_HCDataUnitItem ()

@property (nonatomic, strong) NSRecursiveLock *coreLock;

@end

@implementation BY_HCDataUnitItem

- (id)copyWithZone:(NSZone *)zone
{
    [self lock];
    BY_HCDataUnitItem *obj = [[BY_HCDataUnitItem alloc] init];
...
- (instancetype)initWithPath:(NSString *)path offset:(uint64_t)offset
{
    if (self = [super init]) {
        ...
        self->_relativePath = [BY_HCPathTool converToRelativePath:path];
        self->_absolutePath = [BY_HCPathTool converToAbsoultePath:path];
        self->_offset = offset;
        self->_length = [BY_HCPathTool sizeAtPath:self.absolutePath];
        [self commonInit];
    }
    return self;
}
...
- (instancetype)initWithCoder:(NSCoder *)aDecoder
{
    if (self = [super init]) {
        ...
        self->_absolutePath = [BY_HCPathTool converToAbsoultePath:self.relativePath];
...
- (void)updateLength:(long long)length
{
    [self lock];
    self->_length = length;
    BY_HCLogDataUnitItem(@"%p, Set length : %lld", self, length);
    [self unlock];
}
