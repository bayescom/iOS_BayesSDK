...
@implementation BY_HCPathTool

+ (NSString *)rootDirectory
{
    static NSString *obj = @"BY_BTVHTTPCache";
...
+ (NSString *)logPath
{
    NSString *path = [[self rootDirectory] stringByAppendingPathComponent:@"BY_BTVHTTPCache.log"];
    return [self converToAbsoultePath:path];
}

+ (NSString *)archivePath
{
    NSString *path = [[self rootDirectory] stringByAppendingPathComponent:@"BY_BTVHTTPCache.archive"];
    return [self converToAbsoultePath:path];
}

+ (NSString *)directoryPathWithURL:(NSURL *)URL
{
    NSString *name = [[BY_HCURLTool tool] keyWithURL:URL];
...
{
    NSString *fileName = [[[BY_HCURLTool tool] keyWithURL:URL] stringByAppendingPathExtension:URL.pathExtension];
...
{
    NSString *baseFileName = [[BY_HCURLTool tool] keyWithURL:URL];
    NSString *directoryPath = [self directoryPathWithURL:URL];
    int number = 0;
