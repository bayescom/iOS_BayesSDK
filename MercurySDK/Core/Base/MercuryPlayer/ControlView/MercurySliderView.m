...
@implementation MercurySliderButton
...
/** 滑块 */
@property (nonatomic, strong) MercurySliderButton *sliderBtn;
...
@implementation MercurySliderView
...
- (void)layoutSubviews {
    ...
    self.sliderBtn.mer_centerX = self.bgProgressView.mer_width * self.value;
    
    min_x = 0;
    min_y = 0;
    if (self.sliderBtn.hidden) {
        min_w = self.bgProgressView.mer_width * self.value;
    } else {
        min_w = self.sliderBtn.mer_centerX;
    }
    min_h = self.sliderHeight;
    ...
    min_x = 0;
    min_y = 0;
    min_w = self.bgProgressView.mer_width * self.bufferValue;
    min_h = self.sliderHeight;
    ...
    self.bgProgressView.mer_centerY     = min_view_h * 0.5;
    self.bufferProgressView.mer_centerY = min_view_h * 0.5;
    self.sliderProgressView.mer_centerY = min_view_h * 0.5;
    self.sliderBtn.mer_centerY          = min_view_h * 0.5;
}
...
- (void)setValue:(float)value {
    ...
    _value = value;
    if (self.sliderBtn.hidden) {
        self.sliderProgressView.mer_width = self.bgProgressView.mer_width * value;
    } else {
        self.sliderBtn.mer_centerX = self.bgProgressView.mer_width * value;
        self.sliderProgressView.mer_width = self.sliderBtn.mer_centerX;
    }
}
...
- (void)setBufferValue:(float)bufferValue {
    ...
    self.bufferProgressView.mer_width = self.bgProgressView.mer_width * bufferValue;
}
...
- (void)setSliderHeight:(CGFloat)sliderHeight {
    ...
    self.bgProgressView.mer_height     = sliderHeight;
    self.bufferProgressView.mer_height = sliderHeight;
    self.sliderProgressView.mer_height = sliderHeight;
}
...
- (void)setIsHideSliderBlock:(BOOL)isHideSliderBlock {
    ...
    if (isHideSliderBlock) {
        ...
        self.bgProgressView.mer_left     = 0;
        self.bufferProgressView.mer_left = 0;
        self.sliderProgressView.mer_left = 0;
        self.allowTapped = NO;
    }
}
...
- (void)startAnimating {
    ...
    scaleAnimation.toValue = @(self.mer_width * 10);
...
- (void)sliderBtnDragMoving:(UIButton *)btn point:(CGPoint)touchPoint {
    ...
    CGFloat value = (point.x - btn.mer_width * 0.5) / self.bgProgressView.mer_width;
    // value的值需在0-1之间
...
- (void)tapped:(UITapGestureRecognizer *)tap {
    ...
    // 获取进度
    CGFloat value = (point.x - self.sliderBtn.mer_width * 0.5) * 1.0 / self.bgProgressView.mer_width;
...
- (MercurySliderButton *)sliderBtn {
    if (!_sliderBtn) {
        _sliderBtn = [MercurySliderButton buttonWithType:UIButtonTypeCustom];
