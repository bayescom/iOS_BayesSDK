...
@implementation MercuryImageDownloader

- (void)startDownloadImageWithUrl:(NSString *)url
                         progress:(MercuryDownloadProgressBlock)progress
                         finished:(MercuryDownLoadDataCallBack)finished {
...
+ (NSString *)mer_cachePath;
+ (NSString *)mer_keyForRequest:(NSURLRequest *)request;
...
@interface UIApplication (MercuryCacheImage)

@property (nonatomic, strong, readonly) NSMutableDictionary *mer_cacheFaileTimes;

- (UIImage *)mer_cacheImageForRequest:(NSURLRequest *)request;
- (void)mer_cacheImage:(UIImage *)image forRequest:(NSURLRequest *)request;
- (void)mer_cacheFailRequest:(NSURLRequest *)request;
- (NSUInteger)mer_failTimesForRequest:(NSURLRequest *)request;

@end

@implementation UIApplication (MercuryCacheImage)

- (NSMutableDictionary *)mer_cacheFaileTimes {
...
- (void)setMer_cacheFaileTimes:(NSMutableDictionary *)mer_cacheFaileTimes {
    objc_setAssociatedObject(self, @selector(mer_cacheFaileTimes), mer_cacheFaileTimes, OBJC_ASSOCIATION_RETAIN_NONATOMIC);
}

- (void)mer_clearCache {
    [self.mer_cacheFaileTimes removeAllObjects];
    self.mer_cacheFaileTimes = nil;
}

- (void)mer_clearDiskCaches {
    NSString *directoryPath = [NSString mer_cachePath];
...
- (UIImage *)mer_cacheImageForRequest:(NSURLRequest *)request {
    if (request) {
        NSString *directoryPath = [NSString mer_cachePath];
        NSString *path = [NSString stringWithFormat:@"%@/%@", directoryPath, [NSString cachedFileNameForKey:[NSString mer_keyForRequest:request]]];
...
- (NSUInteger)mer_failTimesForRequest:(NSURLRequest *)request {
    NSNumber *faileTimes = [self.mer_cacheFaileTimes objectForKey:[NSString cachedFileNameForKey:[NSString mer_keyForRequest:request]]];
...
- (void)mer_cacheFailRequest:(NSURLRequest *)request {
    ...
    [self.mer_cacheFaileTimes setObject:@(times) forKey:[NSString cachedFileNameForKey:[NSString mer_keyForRequest:request]]];
}
...
@implementation UIImageView (MercuryCache)

#pragma mark - getter

- (MercuryImageBlock)completion
{
    return objc_getAssociatedObject(self, _cmd);
}

- (MercuryImageDownloader *)imageDownloader
{
    return objc_getAssociatedObject(self, _cmd);
}
...
- (void)setCompletion:(MercuryImageBlock)completion
{
    objc_setAssociatedObject(self, @selector(completion), completion, OBJC_ASSOCIATION_COPY_NONATOMIC);
}

- (void)setImageDownloader:(MercuryImageDownloader *)imageDownloader
{
...
- (void)downloadWithReqeust:(NSURLRequest *)theRequest holder:(UIImage *)holder {
    UIImage *cachedImage = [[UIApplication sharedApplication] mer_cacheImageForRequest:theRequest];
    ...
    if ([[UIApplication sharedApplication] mer_failTimesForRequest:theRequest] >= self.attemptToReloadTimesForFailedURL) {
        return;
    }
    ...
    self.imageDownloader = [[MercuryImageDownloader alloc] init];
        ...
        if (data != nil && error == nil) {
            dispatch_async(dispatch_get_global_queue(0, 0), ^{
                ...
                if (image) {
                    ...
                    [[UIApplication sharedApplication] mer_cacheImage:finalImage forRequest:theRequest];
                } else {
                    [[UIApplication sharedApplication] mer_cacheFailRequest:theRequest];
                }
