...
/**
 ...
 Mercury_FBKVOKeyPath(string.length) => @"length"

 Or even the complex case:

 Mercury_FBKVOKeyPath(string.lowercaseString.length) => @"lowercaseString.length".
 */
#define Mercury_FBKVOKeyPath(KEYPATH) \
...
/**
 ...
 Mercury_FBKVOClassKeyPath(NSString, length) => @"length"
 Mercury_FBKVOClassKeyPath(NSString, lowercaseString.length) => @"lowercaseString.length"
 */
#define Mercury_FBKVOClassKeyPath(CLASS, KEYPATH) \
 ...
 */
extern NSString *const Mercury_FBKVONotificationKeyPathKey;
...
/**
 ...
 @param change The change dictionary which also includes @c Mercury_FBKVONotificationKeyPathKey
 */
typedef void (^FBKVONotificationBlock)(id _Nullable observer, id object, NSDictionary<NSKeyValueChangeKey, id> *change);

/**
 @abstract Mercury_FBKVOController makes Key-Value Observing simpler and safer.
 @discussion Mercury_FBKVOController adds support for handling key-value changes with blocks and custom actions, as well as the NSKeyValueObserving callback. Notification will never message a deallocated observer. Observer removal never throws exceptions, and observers are removed implicitly on controller deallocation. Mercury_FBKVOController is also thread safe. When used in a concurrent environment, it protects observers from possible resurrection and avoids ensuing crash. By default, the controller maintains a strong reference to objects observed.
 */
@interface Mercury_FBKVOController : NSObject
