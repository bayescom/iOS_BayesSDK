...
static NSString *Mercury_describe_option(NSKeyValueObservingOptions option)
{
  switch (option) {
...
static void Mercury_append_option_description(NSMutableString *s, NSUInteger option)
{
  if (0 == s.length) {
    [s appendString:Mercury_describe_option(option)];
  } else {
    [s appendString:@"|"];
    [s appendString:Mercury_describe_option(option)];
  }
...
static NSUInteger Mercury_enumerate_flags(NSUInteger *ptrFlags)
{
...
static NSString *Mercury_describe_options(NSKeyValueObservingOptions options)
{
  NSMutableString *s = [NSMutableString string];
  NSUInteger option;
  while (0 != (option = Mercury_enumerate_flags(&options))) {
    Mercury_append_option_description(s, option);
  }
  return s;
}

#pragma mark Mercury__FBKVOInfo -

typedef NS_ENUM(uint8_t, Mercury_Mercury__FBKVOInfoState) {
  Mercury_Mercury__FBKVOInfoStateInitial = 0,

  // whether the observer registration in Foundation has completed
  Mercury_Mercury__FBKVOInfoStateObserving,

  // whether `unobserve` was called before observer registration in Foundation has completed
  // this could happen when `NSKeyValueObservingOptionInitial` is one of the NSKeyValueObservingOptions
  Mercury_Mercury__FBKVOInfoStateNotObserving,
};

NSString *const Mercury_FBKVONotificationKeyPathKey = @"Mercury_FBKVONotificationKeyPathKey";
 ...
 */
@interface Mercury__FBKVOInfo : NSObject
@end

@implementation Mercury__FBKVOInfo
{
@public
  __weak Mercury_FBKVOController *_controller;
  NSString *_keyPath;
  NSKeyValueObservingOptions _options;
  SEL _action;
  void *_context;
  FBKVONotificationBlock _block;
  Mercury_Mercury__FBKVOInfoState _state;
}

- (instancetype)initWithController:(Mercury_FBKVOController *)controller
...
- (instancetype)initWithController:(Mercury_FBKVOController *)controller keyPath:(NSString *)keyPath options:(NSKeyValueObservingOptions)options block:(FBKVONotificationBlock)block
{
  return [self initWithController:controller keyPath:keyPath options:options block:block action:NULL context:NULL];
}

- (instancetype)initWithController:(Mercury_FBKVOController *)controller keyPath:(NSString *)keyPath options:(NSKeyValueObservingOptions)options action:(SEL)action
{
  return [self initWithController:controller keyPath:keyPath options:options block:NULL action:action context:NULL];
}

- (instancetype)initWithController:(Mercury_FBKVOController *)controller keyPath:(NSString *)keyPath options:(NSKeyValueObservingOptions)options context:(void *)context
{
  return [self initWithController:controller keyPath:keyPath options:options block:NULL action:NULL context:context];
}

- (instancetype)initWithController:(Mercury_FBKVOController *)controller keyPath:(NSString *)keyPath
{
...
- (BOOL)isEqual:(id)object
{
  ...
  return [_keyPath isEqualToString:((Mercury__FBKVOInfo *)object)->_keyPath];
}
...
- (NSString *)debugDescription
{
  ...
  if (0 != _options) {
    [s appendFormat:@" options:%@", Mercury_describe_options(_options)];
  }
  if (NULL != _action) {
...
@implementation Mercury__FBKVOSharedController
{
  NSHashTable<Mercury__FBKVOInfo *> *_infos;
  pthread_mutex_t _mutex;
}

+ (instancetype)sharedController
{
  static Mercury__FBKVOSharedController *_controller = nil;
  static dispatch_once_t onceToken;
  dispatch_once(&onceToken, ^{
    _controller = [[Mercury__FBKVOSharedController alloc] init];
  });
  return _controller;
}
...
- (NSString *)debugDescription
{
  ...
  for (Mercury__FBKVOInfo *info in _infos) {
...
- (void)observe:(id)object info:(nullable Mercury__FBKVOInfo *)info
{
  ...
  if (info->_state == Mercury_Mercury__FBKVOInfoStateInitial) {
    info->_state = Mercury_Mercury__FBKVOInfoStateObserving;
  } else if (info->_state == Mercury_Mercury__FBKVOInfoStateNotObserving) {
    // this could happen when `NSKeyValueObservingOptionInitial` is one of the NSKeyValueObservingOptions,
...
- (void)unobserve:(id)object info:(nullable Mercury__FBKVOInfo *)info
{
  ...
  // remove observer
  if (info->_state == Mercury_Mercury__FBKVOInfoStateObserving) {
    [object removeObserver:self forKeyPath:info->_keyPath context:(void *)info];
  }
  info->_state = Mercury_Mercury__FBKVOInfoStateNotObserving;
}

- (void)unobserve:(id)object infos:(nullable NSSet<Mercury__FBKVOInfo *> *)infos
{
  if (0 == infos.count) {
    return;
  }
  ...
  // unregister info
  pthread_mutex_lock(&_mutex);
  for (Mercury__FBKVOInfo *info in infos) {
    [_infos removeObject:info];
  }
  pthread_mutex_unlock(&_mutex);
                       ...
                       context:(nullable void *)context
{
  ...
  if (nil != info) {
    ...
    Mercury_FBKVOController *controller = info->_controller;
    if (nil != controller) {
      ...
      if (nil != observer) {
        ...
        if (info->_block) {
          ...
          if (keyPath) {
            NSMutableDictionary<NSString *, id> *mChange = [NSMutableDictionary dictionaryWithObject:keyPath forKey:Mercury_FBKVONotificationKeyPathKey];
...
#pragma mark Mercury_FBKVOController -

@implementation Mercury_FBKVOController
{
  NSMapTable<id, NSMutableSet<Mercury__FBKVOInfo *> *> *_objectInfosMap;
...
- (NSString *)debugDescription
{
  ...
  for (id object in _objectInfosMap) {
    ...
    [infos enumerateObjectsUsingBlock:^(Mercury__FBKVOInfo *info, BOOL *stop) {
      [infoDescriptions addObject:info.debugDescription];
    }];
...
- (void)_observe:(id)object info:(Mercury__FBKVOInfo *)info
{
  // lock
  pthread_mutex_lock(&_lock);
  ...
  Mercury__FBKVOInfo *existingInfo = [infos member:info];
  if (nil != existingInfo) {
  ...
  [[Mercury__FBKVOSharedController sharedController] observe:object info:info];
}

- (void)_unobserve:(id)object info:(Mercury__FBKVOInfo *)info
{
  // lock
  pthread_mutex_lock(&_lock);
  ...
  Mercury__FBKVOInfo *registeredInfo = [infos member:info];
  ...
  // unobserve
  [[Mercury__FBKVOSharedController sharedController] unobserve:object info:registeredInfo];
}
...
- (void)_unobserve:(id)object
{
  ...
  // unobserve
  [[Mercury__FBKVOSharedController sharedController] unobserve:object infos:infos];
}
...
- (void)_unobserveAll
{
  ...
  Mercury__FBKVOSharedController *shareController = [Mercury__FBKVOSharedController sharedController];
...
- (void)observe:(nullable id)object keyPath:(NSString *)keyPath options:(NSKeyValueObservingOptions)options block:(FBKVONotificationBlock)block
{
  ...
  // create info
  Mercury__FBKVOInfo *info = [[Mercury__FBKVOInfo alloc] initWithController:self keyPath:keyPath options:options block:block];
...
- (void)observe:(nullable id)object keyPath:(NSString *)keyPath options:(NSKeyValueObservingOptions)options action:(SEL)action
{
  ...
  // create info
  Mercury__FBKVOInfo *info = [[Mercury__FBKVOInfo alloc] initWithController:self keyPath:keyPath options:options action:action];
...
- (void)observe:(nullable id)object keyPath:(NSString *)keyPath options:(NSKeyValueObservingOptions)options context:(nullable void *)context
{
  ...
  // create info
  Mercury__FBKVOInfo *info = [[Mercury__FBKVOInfo alloc] initWithController:self keyPath:keyPath options:options context:context];
...
- (void)unobserve:(nullable id)object keyPath:(NSString *)keyPath
{
  ...
  Mercury__FBKVOInfo *info = [[Mercury__FBKVOInfo alloc] initWithController:self keyPath:keyPath];
